...
class edXUITests: XCTestCase {
    ...
    func testExample() {
        ...
        expectation(for: NSPredicate(format: "exists == true"), evaluatedWith: app.navigationBars["My Nuggets"], handler: nil)
        waitForExpectations(timeout: 20, handler: nil)
        if (app.navigationBars["My Nuggets"].exists) {
            app.navigationBars["My Nuggets"].buttons["Navigation Menu"].tap()
            app.cells.buttons["MY NUGGETS"].tap()
            app.navigationBars["My Nuggets"].buttons["Navigation Menu"].tap()
            app.cells.buttons["MY NUGGETS"].tap()
            app.navigationBars["My Nuggets"].buttons["Navigation Menu"].tap()
            app.cells.buttons["MY NUGGETS"].tap()
