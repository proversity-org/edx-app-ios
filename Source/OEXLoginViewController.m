...
- (void)handleActivationDuringLogin {
    if(self.authProvider != nil) {
        [self.btn_Login applyButtonStyle:[self.environment.styles filledButtonStyle:self.environment.styles.primaryBaseColor] withTitle:[self signInButtonText]];
...
- (IBAction)loginClicked:(id)sender {
    ...
    if([self.tf_EmailID.text length] == 0) {
        ...
        [self.btn_Login applyButtonStyle:[self.environment.styles filledButtonStyle:self.environment.styles.primaryBaseColor] withTitle:[Strings signInButtonTextOnSignIn]];
    }
}
...
- (void)externalLoginWithProvider:(id <OEXExternalAuthProvider>)provider {
    ...
    [self.view setUserInteractionEnabled:NO];
    [self.activityIndicator startAnimating];
    [self.btn_Login applyButtonStyle:[self.environment.styles filledButtonStyle:self.environment.styles.primaryBaseColor] withTitle:[[Strings signInButtonTextOnSignIn] oex_uppercaseStringInCurrentLocale]];
}
